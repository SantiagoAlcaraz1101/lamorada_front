...
const frontend = run('frontend-tests', `${root}/frontend`, ['node_modules/@angular/cli/bin/ng.js', 'test', '--watch=false', '--browsers=ChromeHeadless', '--include=src/app/delivery.paths.spec.ts', '--code-coverage', '--karma-config=../quality/karma.conf.cjs']);
fs.writeFileSync(path.join(reportDir, 'test-run.json'), JSON.stringify({ startedAt, finishedAt: new Date().toISOString(), backend, frontend }, null, 2));
