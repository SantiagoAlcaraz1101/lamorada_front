...
const features = {
  ...
  F07: 'Actualizar el perfil propio', F17: 'Crear publicación',
...
// Ampliación explícita del plan 2.0. Se conservan TODOS los archivos anteriores.
for (const name of ['user-service.js', 'user-controller.js']) {
  entries[`backend/src/modules/user/${name}`].methods.update = ['F07'];
}
directory('backend/src/modules/post', ['F17']);
file('backend/src/modules/post/post-service.js', ['F17'], { createPost: ['F17'], getPosts: ['F17'] }, true);
file('backend/src/modules/post/post-controller.js', ['F17'], { createPost: ['F17'], getPosts: ['F17'] }, true);
// getPosts solo como dependencia del refresco tras crear; no certifica F18 de forma independiente.
entries['backend/src/modules/post/validators/post-validator.js'].lineOverrides = [
  // createPost pasa solo title/content/active al validador, nunca post_id.
  { text: 'if (data.post_id', features: [] },
  { text: 'throw new Error("INVALID ID")', features: [] },
];
directory('frontend/src/app/pages/profile-edit', ['F07']);
entries['frontend/src/app/pages/profile-edit/profile-edit.component.ts'].methods.deleteAccount = [];
for (const ext of ['ts', 'html', 'css']) file(`frontend/src/app/pages/post/post-editor.component.${ext}`, ['F17']);
entries['frontend/src/app/pages/post/post-editor.component.ts'].methods.remove = [];
file('frontend/src/app/services/post-api.service.ts', ['F17'], {
  constructor: ['F17'], authHeaders: ['F17'], create: ['F17'], getAllRawAuth: ['F17'],
}, true);
// PostService no se añade: la pantalla realmente usa PostApiService.
for (const method of ['updateMe', 'updateMeCompat']) entries['frontend/src/app/services/user.service.ts'].methods[method] = ['F07'];
for (const method of ['getMe', 'profileFromToken', 'getToken', 'getAuthHeaders', 'decodeJwt']) {
  entries['frontend/src/app/services/user.service.ts'].methods[method].push('F07');
}
for (const name of ['backend/src/middlewares/jwt-middleware.js', 'backend/src/middlewares/role-middleware.js',
  'frontend/src/app/core/interceptors/auth.interceptor.ts', 'frontend/src/app/guards/auth-role.guard.ts']) {
  entries[name].defaults.push('F07', 'F17');
}
entries['frontend/src/app/pages/post/post-editor.component.html'].blocks = [
  { start: '<div class="actions">', end: '</div>', features: [] },
];
